...
/*SHELL RESTREINT */
/*Fichier des caracteres autorises */
#define CAR_FILE "/etc/restricted-bash/car.txt"
/* repertoires des fichiers contenant les commandes autorisees */
#define PROFILES_DIR "/etc/restricted-bash/"
/*GID minimum des utilisateurs sousmis aux restrictions */
#define GID_MIN 1000
/*SHELL RESTREINT */
...
int test_regex_opt(char * str_regex, char * commande, char flag){
/* fonction testant les options d'une ligne de commande */
/* retourne 0 si pas d interdiction */
/* les regex dans les commandes autorisees permettent de limiter les options dangeureuses*/
/* retourne 1 si l'option n'est pas autorisée */
/* retourne 2 si toutes la ligne est matchée */
	...
	int err;
	regex_t preg ;
	char *commande2 = (char*)malloc(sizeof (char)* (strlen(commande)+1));
	char *temp = (char*)malloc(sizeof (char)* strlen(commande)+1);
	char *str_regex_cpy = (char*)malloc(sizeof (char)* (strlen(str_regex)+1));
	int match;
	int logg;
	

	
	if (commande2 == NULL) {printf("erreur alloc 1\n");}
	if (temp == NULL) {printf("erreur alloc 2\n");}
	if (str_regex_cpy == NULL) {printf("erreur alloc str_regex_cpy\n");}
	
		
// si pas de regex alors pas de pb on renvoie 0
	if   (str_regex == NULL){
		return 0;
	}
    
   strncpy(commande2,commande,strlen(commande));//on travaille sur une copie
   commande2[strlen(commande)]='\0';
	strncpy(str_regex_cpy,str_regex,strlen(str_regex));//on travaille sur une copie  
	str_regex_cpy[strlen(str_regex)]   ='\0';
  
   temp=strsep(&commande2," \t"); // on le fait une fois a vide pour virer le binaire/executable et l espace

	if (commande2 == NULL) {return 0;}//pas d expression d option
	
	// on teste pour savoir si toute la ligne horsmis le binaire est matchee 
	
	if ( flag == 'O'){

		err = regcomp (&preg, str_regex_cpy, REG_NOSUB | REG_EXTENDED);
		if (err == 0) {
				match = regexec (&preg, commande2, 0, NULL, 0);
				if (match == 0){
						logg=log_file("CODE 11.1 - tous les arguments sont matches :\n", commande);
						regfree(&preg);
					return 2;
				}
				else
				 if (match == REG_NOMATCH){
						logg=log_file("CODE 11.2 - tous les arguments ne sont matches de maniere exhautive :\n", commande);
						regfree(&preg);
						return 1;
				}
				return 0;
...
if (( flag == 'N') || ( flag == 'I')){
	...
	while (commande2 != NULL){
	
	    temp=strsep(&commande2," \t");	
	    
	    
	    		
	    if (temp != NULL){
			if ( strncmp("-",temp,1) == 0){
				if ( str_regex_cpy != NULL ){
					
					
					/*printf("str_regex : %s\n",str_regex_cpy);
					for(int i=0;i<=strlen(str_regex_cpy);i++){
						printf("%02X",(unsigned char)str_regex_cpy[i]);
				 	 } 
				  	printf("\n");							
					...
					*/
					...
					if ((err = regcomp (&preg, str_regex_cpy, REG_NOSUB | REG_EXTENDED))){
					...
					if (err == 0) {
						...
						if (match == 0){
							//expression rationnelle matche
				
							return 1;
						}
						else 
							if (match == REG_NOMATCH){
								//pas de resultat a l expression rationnel
								logg=log_file("CODE 99 - option match\n : ", temp); 
							}
					...
					return 0;
					//printf("arguments : %s \n",temp);
					logg=log_file("CODE 99 - option match : ", temp); 
				}
		}
		else
			{
				log_file("CODE 99 - option match : ", "erreur :ce n est pas une option\n");
			}
	}
	}
...
int car_autorise(char car){
// on retourne 1 si le car est dans la liste autorisee dans le fichier et 0 sinon
// Les caracteres autorises sur la ligne de commande sont dans le fichier car.txt
int test=1;
char buffer[2048];
...
int err=0;
	...
	if (fgets(buffer, sizeof(buffer), fp) == NULL){ err=1;} 
...
int nettoie(char *chaine)
{ int logg;
	/* fonction qui verifie que la ligne de commande fournie est conforme a la liste des caracteres autorises */
	// retourne 0 si chaine non autorise
	// retourne 1 si chaine propre
	
    unsigned long i = 0;
    unsigned long x = 0;
    char c;

  //printf("chaine non nettoyee : %s \n",chaine);
    ...
    while ((c = chaine[i++]) != '\0') {
		...
		//	printf("%c",c);
			...
			printf("car %c non autorise a la position :%lu \n",c,x);
			logg=log_file("CODE 01 - Commande lancee par l'utilisateur n'est pas saine : ", chaine); 
			return 0;	 
		}	
   }
    chaine[x] = '\0';
 logg=log_file("CODE 02 - Commande lancee par l'utilisateur est saine :", chaine);  
...
int ctl_gid(){
//fonction permettant de tester si le gid du user possede un fichier nomme numero_gid.txt. Ce fichier contiendra les commandes autorisees  
  int logg;
  uid_t uid = geteuid ();
  gid_t gid = getegid ();
  FILE *fp;
  ...
  snprintf(tmp_path,254,"%s%d.txt",PROFILES_DIR,gid);
  ...
  fp = fopen (tmp_path,"r");
    if (fp == NULL) {
 	    ...
 	    logg=log_file("CODE 03 - Le fichier de commande de l\'utilisateur n'existe pas :\n", tmp_path);
...
int log_file( char *chaine1, char *chaine);

int log_file( char *chaine1, char *chaine2){
/* fonction ecrivant deux chaines dans le fichier syslog */
time_t date;
char *string;
date = time(NULL); 

string = (char*) malloc(10240 * sizeof(char));

if (strlen(chaine2) < 2048){ 
	//printf("chaine 2 : %s\n",chaine2);
	snprintf(string,2048,"%s - uid:%d - %s -  %s  \n",strtok(ctime(&date),"\n"),getuid(), chaine1, chaine2);
	openlog ("- Bash Restreint - ", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_USER);
    syslog (LOG_DEBUG,string);
	closelog();
	
}
else{
 	openlog ("- Bash Restreint - ERROR LOG", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_USER);
    syslog (LOG_DEBUG,"Chaine trop longue");
	closelog();
}
...
int ctl_cmd_gid(char * command){
//retourne 1 si la commande est autorise dans le fichier GID
//retourne 0 si la commande n est pas autorise dans fichier GID
...
int LMAX = 2048;
int taille_max_file=200;
char tmp_path[2048]="";
char *ligne=(char*)malloc(sizeof(char) * LMAX);
char t_cmd[taille_max_file][2048];
char *arg0cmd=(char*)malloc(sizeof(char) * LMAX);
char *arg0cmdfile=(char*)malloc(sizeof(char) * LMAX);
char *arg0cmdnotmodified=(char*)malloc(sizeof(char) * LMAX);
char * save_command=(char*)malloc(sizeof(char) * strlen(command)+1);
char * temp_test_realname=(char*)malloc(sizeof(char) * strlen(command)+1);
//char * save_sep=(char*)malloc(sizeof(char) * strlen(command)+1); // pour garder la trace du separateur d option
char *tmp_opt=(char*)malloc(sizeof(char) * LMAX);
char realname[PATH_MAX];
char *flag_sep=malloc(sizeof(char) * strlen(command)+1);
int int_regexcomp;
int logg;

   snprintf(save_command,strlen(command)+1,"%s",command);//car command est modifiee par strsep pour le controle du basename
      	      
   snprintf(temp_test_realname,strlen(command)+1,"%s",command);//car command est modifiee par strsep pour le controle du realpath
   	
   
	snprintf(tmp_path,LMAX,"%s%d.txt",PROFILES_DIR,gid); //A TESTER PEUT ETRE
	
	fp = fopen (tmp_path,"r");
   if (fp == NULL) {
		logg=log_file("CODE 04 - Le fichier n'existe pas pour l'utilisateur", tmp_path); 
      return 0;
   }
   
   // On utilise pas mal de variable car strsep modifie la variable en parametre
	arg0cmd = basename(strsep(&command, " \t"));//premier argument de la ligne a verifier
    arg0cmdnotmodified = strsep(&temp_test_realname," \t");
	//on recupere le premier argument pour test le chemin. Si un utilisateur se cree une copie local du binaire avec le meme nom
   ...
   ssize_t nread;
   size_t len = 0;
	while ((nread = getline(&ligne, &len, fp)) != -1) {
		removeStringTrailingNewline(ligne);
		//printf("ligne : %s\n",ligne);
	//	snprintf(save_sep,strlen(ligne),"%s",ligne);
		
		tmp_opt=strsep(&ligne,"#");//commande issue du fichier de commande		
		
	
		if (realpath(arg0cmdnotmodified,realname) == NULL){
			//le binaire existe-t-il ?
			
			logg=log_file("CODE 10 - Le chemin de la commande  n\'existe  des commandes valides pour l\'utilisateur :  ", save_command); 
	    	return 0;
		}
		
		
				
		if (strcmp(tmp_opt,realpath(arg0cmdnotmodified,realname)) == 0){

			//comparer TMP_OPT et   basename(strsep(&command, " \t"));
			//utiliser fonction  realpath() char realname[PATH_MAX];realpath(argument[i],realname)
			if (ligne == NULL){
					return 1;
			}
			...
			if (ligne != NULL){
					//alors des options sont a analysé
					flag_sep=strsep(&ligne,"#"); // on contient le flag et ligne le reste de la regex
...
*/
			    ...
			    int_regexcomp=test_regex_opt(ligne,save_command,flag_sep[0]);
					
					
						


					if (( int_regexcomp == 2) && (flag_sep[0] == 'O')) {
						
						// si 1 c est qu une option est autorisee
						logg=log_file("CODE 05.5 - ligne de commande autorise pour l\'utilisateur ", save_command); 
				   ...
				   if (( int_regexcomp == 1) && (flag_sep[0] == 'N')) {
						
						// si 1 c est qu une option est interdite
						logg=log_file("CODE 05.1 - une option de la commande est dans la liste des options interdites pour l\'utilisateur  ", save_command); 
				   ...
				   if (( int_regexcomp == 0) && (flag_sep[0] == 'I')) {
						
						// si 1 c est qu une option est interdite
						logg=log_file("CODE 05.2 - une option de la commande est dans la liste des options interdites pour l\'utilisateur  ", save_command); 
				    ...
				    if (( int_regexcomp == 1) && (flag_sep[0] == 'I')) {
						
						// si 0 et sep = I alors  c est qu une option est autorisee
						logg=log_file("CODE 05.3 - une option de la commande est dans la liste des options autorisee pour l\'utilisateur ", save_command); 
				    ...
				    if (( int_regexcomp == 0) && (flag_sep[0] == 'N')) {
						
						// si 0 et sep = I alors  c est qu une option est autorisee
						logg=log_file("CODE 05.4 - une option de la commande est dans la liste des options autorisee pour l utilisateur ", save_command); 
	...
	logg=log_file("CODE 06 - La commande n\'est pas dans la liste des commandes valides pour l\'utilisateur  ", save_command); 
...
char * argv_to_string(char * commande,char ** args){
// convertir la liste des arguments en strings

char *tempo=(char*)malloc(sizeof(char)*55600);
char * rtn=(char*)malloc(sizeof(char)*55600);
//char *tempo2;
int taille=0;
...
//printf("commande : %s\n", commande);
	...
	while (args[nb_arg]!= NULL){
		//printf(" argv_to_string %i : %s\n",nb_arg,args[nb_arg]);
		taille=taille+strlen(args[nb_arg]);
		nb_arg++;
	}
	//printf("nb arg : %i\n",nb_arg);
	if (taille > 2048) {
		logg=log_file("CODE 07 - La commande trop grande ou trop d option\n", commande);
		return "";
	}
	

	//on passe le premier parametre pour prendre la commande avec le chemin absolu et non pas la commande de la liste d arguments
	
	strncpy(tempo,commande,strlen(commande));	
	tempo[strlen(commande)]='\0';
	rtn=strncat(tempo," ",strlen(commande));

	i++;

	while (i < nb_arg){

		rtn=strncat(rtn,args[i],strlen(args[i]));
...
int controle_bash(char * command){
	// fonction principale qui appelle les autres
	// celle ci est dans la fonction d origine du code source 

	 char  *tempo=(char*)malloc(sizeof(char)*2048);
	int logg;
	
				 
	
	/*
	printf("control bash command :%s\n",command);
	...
	*/
	...
	if( strlen(command) < 2048){ 
		//on verifie si le gid a bien un fichier de commande autorise
		if (ctl_gid() == 1){
			//printf("strlen commande %lu\n",strlen(command));
			//on filtre la cmd
			strncpy(tempo,command,strlen(command));
			tempo[strlen(command)]='\0';
			/*
					printf("tempo : %s\n",tempo);
						for(int i=0;i<=strlen(tempo);i++){
						printf("%02X",(unsigned char)tempo[i]);
				 	 } 
				  	printf("\n");	
			*/
		
			if (nettoie(tempo) == 1){
				if (ctl_cmd_gid(tempo) == 1){
					
					logg=log_file("CODE 08 - La commande est autorisee pour l'utilisateur", tempo); 
...
/*SHELL RESTREINT */
/* Version 2 : a faire  : harmoniser les codes retours des fonctions*/
/*on insere ici la verification de la commande */
/* les iteractions avec le fonctionnement de bash est minimaliste */
/* puisque l'on fait juste un exit en cas d interdiction de la commande, sinon le bash se deroule normalement */


if (getgid()>=GID_MIN){

		if( controle_bash(argv_to_string(command,args)) == 0){
			exit (1);    
		}
}
